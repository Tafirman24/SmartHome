...
bool isOverload = false; // Flag untuk status overload
bool limitChanged = false; // Tandai apakah limit telah diubah
const unsigned long interval = 60000; // Jeda 5 detik
...
void setLimit(int newLimitPwr) {
    ...
    if (power > newLimitPwr) {
        if (!isOverload) {
            ...
            lcd.print("Overload");
            OPOFF(); // Matikan relay
            isOverload = true; // Set flag overload
            previousMillisLimit = currentMillisLimit; // Set waktu mulai overload
        }
    } else {
        if (isOverload) {
            if (currentMillisLimit - previousMillisLimit >= interval) {
                OPON(); // Nyalakan relay setelah interval
                lcd.setCursor(11, 2); 
                //lcd.print("Normal");
                isOverload = false; // Reset flag overload
            }
        } else {
            // Jika daya tidak melebihi batas dan tidak dalam kondisi overload
            //lcd.clear();
            OPON();
            lcd.setCursor(11, 2); 
            //lcd.print("Normal");
            previousMillisLimit = currentMillisLimit; // Reset timer saat dalam kondisi normal
        }
    }
}
